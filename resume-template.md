---
name: Your Name
email: your.email@example.com
phone: +1 (555) 123-4567
location: City, State
linkedin: linkedin.com/in/yourprofile
github: github.com/yourusername
website: yourwebsite.com
---

# Your Name

## Summary

A 2-3 sentence summary of who you are professionally. Focus on your key strengths, years of experience, and what you're known for. This should be compelling enough to make someone want to read the rest.

Example: "Operations leader with 10+ years building and scaling teams at high-growth startups. Known for turning chaos into systems and ambiguity into clarity. Passionate about using technology to solve human problems."

## Experience

### Job Title | Company Name
**Location** | Month Year - Present

- Lead with impact: Start each bullet with a strong action verb and quantify results when possible
- Example: "Scaled customer success team from 3 to 15, reducing churn by 40% and increasing NPS from 32 to 67"
- Focus on outcomes, not just activities - what changed because of your work?
- 3-5 bullets per role is usually enough for recent positions, fewer for older ones

### Previous Job Title | Previous Company
**Location** | Month Year - Month Year

- Another accomplishment with measurable impact
- Skills or technologies you used and why they mattered
- Any notable projects, initiatives, or recognition

### Earlier Role | Company Name
**Location** | Month Year - Month Year

- Key accomplishment
- Another key accomplishment

## Skills

**Technical:** List relevant technical skills, tools, and technologies
**Leadership:** Team building, strategic planning, cross-functional collaboration
**Domain:** Industry-specific knowledge, certifications, methodologies

## Education

### Degree, Major | University Name
Year of Graduation

---

## Tips for Your Resume

1. **Keep it to 1-2 pages** - Recruiters spend 6 seconds on initial scan
2. **Quantify everything you can** - Numbers make impact tangible
3. **Tailor for each application** - That's what TL;DResume is for!
4. **Use consistent formatting** - Same fonts, spacing, bullet styles
5. **Lead with your strongest stuff** - Most recent/relevant experience first
6. **Remove this tips section** - It's just here to help you get started!

---

*Delete everything below the Education section (including this tips section) before using this template.*
